      ...
      <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-7">
        ...
        <div className="flex justify-between items-center p-5 bg-white rounded-md gap-3">
          <div className="flex flex-col justify-start items-start text-gray-600">
            ...
            <h2 className="text-xl font-semibold">
              {formateCurrency(availableAmount)}
            </h2>
          </div>
              ...
              <Table.Body className="divide-y">
                {pendingWithdrawalRequest &&
                  pendingWithdrawalRequest.map((r, index) => (
                    ...
                    >
                      <Table.Cell className="whitespace-nowrap overflow-hidden text-ellipsis truncate font-medium text-gray-900 dark:text-white">
                        {index + 1}
                      </Table.Cell>
                      <Table.Cell className="whitespace-nowrap overflow-hidden text-ellipsis truncate font-medium text-gray-900 dark:text-white">
                        {formateCurrency(r.amount)}
                      </Table.Cell>
                      <Table.Cell>
                        {r.status === "pending" && (
                          <Badge
                ...
                {successWithdrawalRequest &&
                  successWithdrawalRequest.map((s, index) => (
                    ...
                    >
                      ...
                      <Table.Cell>{formateCurrency(s.amount)}</Table.Cell>
                      <Table.Cell>
                        <Badge
                          color="success"
                          className="inline-block px-2 py-1"
