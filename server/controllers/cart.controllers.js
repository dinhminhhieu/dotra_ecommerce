...
class cartController {
  ...
  get_cart = async (req, res) => {
    ...
    try {
      ...
      // Hết hàng
      let out_of_stock = cart.filter(
        (item) => item.products[0].quantity < item.quantity
      );
      for (let i = 0; i < out_of_stock.length; i++) {
        cart_product_count += out_of_stock[i].quantity;
      }
      // Còn sản phẩm
      const quantity_product = cart.filter(
        (item) => item.products[0].quantity >= item.quantity
      );
      ...
      let unique = [
        ...new Set(
          quantity_product.map((item) => item.products[0].sellerId.toString())
        ),
      ];
        ...
        for (let j = 0; j < quantity_product.length; j++) {
          ...
          if (unique[i] === temp_product.sellerId.toString()) {
            ...
            if (temp_product.discount !== 0) {
              price =
                temp_product.price -
                Math.floor((temp_product.price * temp_product.discount) / 100);
            } else {
  ...
  // 4. Tăng số lượng sản phẩm trong giỏ hàng
  ...
  // 5. Giảm số lượng sản phẩm trong giỏ hàng
  ...
  apply_coupons = async (req, res) => {
    ...
    try {
      ...
      if (!coupons) {
        ...
        response(res, httpStatusCode.Ok, {
          message: successMessage.APPLY_COUPONS_SUCCESS,
          data: coupons.coupons_price,
        });
      }
    } catch (error) {
      response(res, httpStatusCode.InternalServerError, {
