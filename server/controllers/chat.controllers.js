...
class chatController {
  // 1. Lấy danh sách hiển thị seller và customer chat
    ...
    try {
      if (sellerId !== "") {
        ...
        const checkCustomer = await sellerToCustomerModel.findOne({
          myId: sellerId,
          myFriends: { $elemMatch: { friendId: customerId } },
        });
        ...
        const messages = await messageSellerAndCustomer.find({
          $or: [
            { receiverId: sellerId, senderId: customerId },
            { receiverId: customerId, senderId: sellerId },
          ],
        });
        ...
        // Tìm bạn bè hiện tại
        const current_friend = my_friends.myFriends.find(
          (friend) => friend.friendId === sellerId
        );
  ...
  // 2. Gửi tin nhắn từ khách hàng sang seller
  send_message = async (req, res) => {
    ...
    form.parse(req, async (err, field, files) => {
      ...
      try {
        ...
        let index = my_friends.findIndex(
          (friend) => friend.friendId === sellerId
        );
        while (index > 0) {
        ...
        let index1 = my_friends1.findIndex(
          (friend) => friend.friendId === customerId
        );
        while (index1 > 0) {
        ...
        response(res, httpStatusCode.Created, {
          message: successMessage.SEND_MESSAGE_SUCCESS,
          messages,
        });
      } catch (error) {
        response(res, httpStatusCode.InternalServerError, {
  ...
  // 3. Lấy customer đang trò chuyện trả về seller
  ...
  // 4. Lấy tin nhắn customer trả về seller
  ...
  // 5. Gửi tin nhắn từ seller đến customer
  ...
  // 6. Gửi tin nhắn từ seller sang admin vaf admin sang seller
  ...
  // 7. Lấy tin nhắn của admin đã nhắn cho seller
  ...
  // 8. Lấy danh sách seller đang chat
  ...
  // 9. Lấy tin nhắn của seller đã nhắn cho admin
